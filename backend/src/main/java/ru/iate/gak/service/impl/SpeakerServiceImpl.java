...
public class SpeakerServiceImpl implements SpeakerService {
	...
	public List<File> getSpeakerProtocolsForTodaySpeakersOfGroup(String group) {
		...
		logger.info("Запрашиваются протоколы для группы " + group + " с " + currentDayInUTC.minusDays(1) + " UTC" + " по " + currentDayInUTC + " UTC");
			...
			if (speakerEntity != null) {
				if (speakerEntity.getStudent() != null) {
					if (speakerEntity.getStudent().getDiplom() != null) {
						...
						logger.info("Статус диплома " + speakerEntity.getStudent().getDiplom().getId() + " изменен на " + status);
